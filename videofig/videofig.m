...
%   João Filipe Henriques, 2010
    ...
    function save_gif()
        answers = inputdlg({'Enter filename:', 'Save with scrollbar (y/n):', 'Loop with reverse (y/n):'}, ...
            'Save as GIF', [1 128; 1 128; 1 128], {'.gif', 'n', 'n'});
        if isempty(answers); return; end
        
        outfile = answers{1};

        switch answers{2}
            case 'n'; frameFunc = @() getframe(axes_handle); 
            otherwise; frameFunc = @() getframe(fig_handle); 
        end

        switch answers{3}
            case 'n'; frameIdx = 1:num_frames; 
            otherwise; frameIdx = [1:num_frames, (num_frames-1):-1:2];
        end
        ...
        for ii = frameIdx
            ...
            if ii == 1
                imwrite(cdata, map, outfile, 'gif', 'LoopCount', Inf, 'DelayTime', 1/play_fps);
            else
                imwrite(cdata, map, outfile, 'gif', 'WriteMode', 'append', 'DelayTime', 1/play_fps);
            end
