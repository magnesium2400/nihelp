...
%% Examples
%   n = noise('powerlaw1', struct('sz', 20, 'alpha', 1));
...
switch type
    case 'powerlaw1'
        ...
        v = ifft(ifftshift(ff, 1)); % power-law noise vector
        vScaled = rescale(real(v),-1,1); % scaled power-law noise [-1,1]
        out = vScaled.';
    case 'powerlaw2'
        if isUnemptyField(opts, 'seed'); try rng(opts.seed); catch; end; end
        n = opts.sz;
        ...
        d = ((1:n)-(n/2)-1).^2;
        dd = sqrt(d(:) + d(:)');
        filt = dd .^ (-a/2); % frequency filter
        filt(isinf(filt))=1; 
        ff = mf .* filt;
        out = rescale(real(ifft2(ifftshift(ff))), -1, 1);
    otherwise
