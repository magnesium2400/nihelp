function [cells, cellsDone] = triangulation2tetrahedra(faces)
...
while any(~cellsDone)
    ...
    for ii = width(cells):-1:1 % loop over all faces of current cell
        ...
        n = getNeighbours(faces, currentFace);

        if length(n) == 2
            % currentCellFlag = 1;
            newCell = [currentFace, n(n~=currentCell(ii))];
...
for ii = length(currentNeighbours):-1:1
    for jj = 1:width(faces)
        ...
        % if ~any(all(ismember(tri,[a,currentNeighbours(ii)]),2))
